  ...
  class AgentRunner {
    ...
    _buildExecutionInitialInput({ job, blocks, settings }) {
      ...
      const systemText = [
        ...
        'Prefer contiguous units: 10-15 sequential blocks from the same category per call when possible.',
        'For multi-block units, translation MUST be structured (items with blockId/text) and mapped back to every block.',
    ...
    async _forceExecutionProgress({ job, blocks, settings, loop, reason = 'fallback' } = {}) {
      ...
      try {
        ...
        const unitsOut = await this.toolRegistry.execute({
          ...
          arguments: { limit: 3, prefer: 'mixed' },
          job: safeJob,
          blocks,
          settings,
