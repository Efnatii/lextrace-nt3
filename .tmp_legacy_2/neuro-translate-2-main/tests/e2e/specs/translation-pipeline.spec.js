...
test.describe('MV3 translation e2e', () => {
  test('C1: scan-only preanalysis -> planning -> awaiting_categories -> execution', async ({ app }) => {
    ...
    const planningOrAwaiting = await app.waitForState(
      ...
      { timeoutMs: app.isRealMode ? 180000 : 70000, label: 'planning_or_awaiting' }
    );
    ...
    const awaiting = planningOrAwaiting && planningOrAwaiting.jobStatus === 'awaiting_categories'
      ...
      : await app.waitForState(
        ...
        { timeoutMs: app.isRealMode ? 180000 : 70000, label: 'awaiting_categories' }
      );

    await popup.locator('[data-tab="status"]').click();
    await expect(popup.locator('[data-tab-panel="status"]')).toBeVisible();
    ...
    const startSelectedBtn = popup.locator('[data-action="start-selected-categories"]');
    await expect(startSelectedBtn).toBeVisible();
    if (await startSelectedBtn.isDisabled()) {
      const firstToggle = popup.locator('[data-section="category-chooser-list"] input[type="checkbox"]').first();
      await firstToggle.check({ force: true });
    }
    await startSelectedBtn.click();
    let running = null;
    try {
      running = await app.waitForState(
        tabId,
        (state) => state && state.jobStatus === 'running',
        { timeoutMs: app.isRealMode ? 45000 : 12000, label: 'running after popup click' }
      );
    } catch (_) {
      ...
      running = await app.waitForState(
        tabId,
        (state) => state && state.jobStatus === 'running',
        { timeoutMs: app.isRealMode ? 180000 : 70000, label: 'running after explicit category select' }
      );
    }
    expect(running && running.jobStatus).toBe('running');

    await waitForStreamingDomChange(app, site, tabId, '#simple-intro', originalIntro, {
      timeoutMs: app.isRealMode ? 180000 : 90000,
      pollMs: 180
    });
    ...
    const done = await app.waitForState(
      ...
      { timeoutMs: app.isRealMode ? 260000 : 140000, label: 'done' }
    );
    ...
    }) || getChecklistStatus(checklist, 'scanned') === 'done' || getChecklistStatus(checklist, 'analyze_page') === 'done';
  ...
  test('C3: cancel mid-stream stops job without RUNNING forever', async ({ app }) => {
    ...
    const size = app.isRealMode ? 320 : 900;
    ...
    await app.waitForState(
      tabId,
      (state) => state && (
        (state.jobStatus === 'running' && hasTranslatedBlocks(state))
        || state.jobStatus === 'done'
      ),
      { timeoutMs: 140000, intervalMs: 1200, label: 'running with first translated block|done' }
    );
    ...
    const cancelled = await app.waitForState(
      tabId,
      (state) => state && (state.jobStatus === 'cancelled' || state.jobStatus === 'done'),
      { timeoutMs: 90000, label: 'cancelled|done' }
    );
    ...
    expect(after && (after.jobStatus === 'cancelled' || after.jobStatus === 'done')).toBeTruthy();
  ...
  test('C5: reload mid-run restore does not get stuck in RUNNING', async ({ app }) => {
    ...
    await beginTranslationFlow(app, tabId, {
      timeoutMs: app.isRealMode ? 180000 : 90000,
      runningTimeoutMs: app.isRealMode ? 180000 : 70000,
      maxCategories: 4
    });
    await app.waitForState(
      tabId,
      (state) => state && state.jobStatus === 'running' && hasTranslatedBlocks(state),
      { timeoutMs: app.isRealMode ? 220000 : 120000, label: 'running before reload' }
    );

    await site.reload({ waitUntil: 'domcontentloaded' });
    await app.kickScheduler(tabId).catch(() => null);

    const terminal = await waitForTerminal(app, tabId, app.isRealMode ? 260000 : 150000);
  ...
  test('C7: memory restore on second open', async ({ app }) => {
    ...
    await beginTranslationFlow(app, firstTabId, {
      timeoutMs: app.isRealMode ? 180000 : 80000,
      runningTimeoutMs: app.isRealMode ? 180000 : 70000,
      maxCategories: 4
    });
    await app.waitForState(firstTabId, (state) => state && state.jobStatus === 'done', {
      timeoutMs: app.isRealMode ? 260000 : 130000,
      label: 'first done'
    });
    ...
    const secondState = await app.waitForState(
      ...
      { timeoutMs: app.isRealMode ? 180000 : 70000, label: 'memory restore stage' }
    );
