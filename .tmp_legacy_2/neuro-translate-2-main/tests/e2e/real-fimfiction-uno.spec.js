...
async function pollActiveJobId(app, tabId, testInfo, { timeoutMs = 45000, pollMs = 250 } = {}) {
  ...
  while ((Date.now() - startedAt) < timeoutMs) {
    ...
    const snapshot = await app.sendBgMessage('BG_TEST_GET_ACTIVE_JOB_STATE', { tabId }).catch(() => null);
    if (snapshot && snapshot.ok) {
      const fromJob = snapshot.job && snapshot.job.id ? String(snapshot.job.id) : null;
      const fromActive = snapshot.activeJobId ? String(snapshot.activeJobId) : null;
      const fromLast = snapshot.lastJobId ? String(snapshot.lastJobId) : null;
      const candidate = fromActive || fromJob || fromLast;
      if (candidate) {
        rememberRuntime(testInfo, { tabId, jobId: candidate });
        return candidate;
      }
    }
    await sleep(pollMs);
