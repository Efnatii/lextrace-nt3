...
function normalizeBrowserFlavor(value) {
  return String(value || '').trim().toLowerCase() === 'edge-stable'
    ? 'edge-stable'
    : 'chromium';
}
...
async function resolveEdgeStableExecutablePath() {
  const fromEnv = String(process.env.EDGE_EXECUTABLE_PATH || '').trim();
...
async function waitForServiceWorker(context, timeoutMs) {
  let sw = context && typeof context.serviceWorkers === 'function'
    ? context.serviceWorkers()[0] || null
    : null;
  if (!sw) {
    sw = await context.waitForEvent('serviceworker', { timeout: timeoutMs }).catch(() => null);
  }
  return sw || null;
}

async function launchExtensionContext({ userDataDir, headless = true, flavor = 'chromium' } = {}) {
  const requestedFlavor = normalizeBrowserFlavor(flavor);
  const launchArgs = [
  ...
  if (requestedFlavor === 'edge-stable') {
    const edgeExecutablePath = await resolveEdgeStableExecutablePath();
    if (edgeExecutablePath) {
      // eslint-disable-next-line no-console
      console.log(`[e2e] trying edge-stable executable: ${edgeExecutablePath}`);
      const edgeContext = await chromium.launchPersistentContext(userDataDir, {
        ...commonOptions,
        executablePath: edgeExecutablePath
      }).catch(() => null);
      if (edgeContext) {
        const sw = await waitForServiceWorker(edgeContext, 10_000);
        if (sw) {
          return {
            context: edgeContext,
            requestedFlavor,
            usedFlavor: 'edge-stable',
            fallbackUsed: false
          };
        }
        await edgeContext.close().catch(() => {});
        // eslint-disable-next-line no-console
        console.warn('[e2e] edge-stable launched but service worker was not detected; fallback to bundled chromium');
      } else {
        // eslint-disable-next-line no-console
        console.warn('[e2e] edge-stable launch failed; fallback to bundled chromium');
      }
    } else {
      // eslint-disable-next-line no-console
      console.warn('[e2e] edge-stable executable not found; fallback to bundled chromium');
    }
  }

  const chromiumContext = await chromium.launchPersistentContext(userDataDir, {
    ...commonOptions,
    channel: 'chromium'
  });
  return {
    ...
    requestedFlavor,
    usedFlavor: 'chromium',
    fallbackUsed: requestedFlavor === 'edge-stable'
  };
}
...
async function sendUiCommand(page, { command, payload = {}, tabId = null, timeoutMs = 12000 } = {}) {
  return page.evaluate(async ({ commandName, commandPayload, resolvedTabId, timeoutMs }) => {
    ...
    resolvedTabId: tabId,
    timeoutMs: Number.isFinite(Number(timeoutMs)) ? Number(timeoutMs) : 12000
  });
}

async function sendBgRuntimeMessage(page, { type, payload = {}, timeoutMs = 12000 } = {}) {
...
const test = base.extend({
  ...
  context: async ({ servers }, use, testInfo) => {
    ...
    const headless = testInfo && testInfo.project && testInfo.project.use
      ...
      : true;
    const requestedFlavor = normalizeBrowserFlavor(process.env.BROWSER_FLAVOR || 'chromium');
    ...
    // eslint-disable-next-line no-console
    console.log(`[e2e] browser flavor requested=${launched.requestedFlavor} used=${launched.usedFlavor}${launched.fallbackUsed ? ' (fallback)' : ''}`);
    await use(context);
    await context.close();
    await sleep(300);
  },
  ...
  app: async ({ context, extensionId, servers }, use, testInfo) => {
    ...
    const app = {
      ...
      openPopupPage: async (tabId) => {
        ...
        await page.goto(url, { waitUntil: 'domcontentloaded' });
        await page.waitForSelector('[data-tab="status"]', { timeout: 12000 });
        return page;
      },
      ...
      sendBgMessage: async (type, payload = {}, options = {}) => {
        ...
        if (normalizedType.startsWith('BG_TEST_')) {
          const tabId = Number.isFinite(Number(normalizedPayload.tabId))
            ...
            : null;
          const viaCommand = await app.sendCommand(normalizedType, normalizedPayload, tabId, { timeoutMs });
      ...
      sendCommand: async (command, payload = {}, tabId = null, options = {}) => {
        const timeoutMs = Number.isFinite(Number(options && options.timeoutMs))
          ? Number(options.timeoutMs)
          : 12000;
        for (let attempt = 1; attempt <= 3; attempt += 1) {
          ...
          try {
            const helper = await ensureHelperPage();
            result = await sendUiCommand(helper, { command, payload, tabId, timeoutMs });
          } catch (error) {
            if (helperPage && !helperPage.isClosed()) {
              await helperPage.close().catch(() => {});
            }
      ...
      configureTestBackend: async ({ mode = TEST_MODE } = {}) => {
        ...
        let ping = null;
        for (let attempt = 1; attempt <= 3; attempt += 1) {
          ping = await app.sendCommand('BG_TEST_CONNECTION', { timeoutMs: 90000 }, null, { timeoutMs: 120000 });
          if (ping && ping.ok === true) {
            break;
          }
          if (attempt < 3) {
