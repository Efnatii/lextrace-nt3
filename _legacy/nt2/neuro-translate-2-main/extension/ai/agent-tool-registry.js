  ...
  class AgentToolRegistry {
    constructor({
      ...
      this.setAgentCategoryRecommendations = typeof setAgentCategoryRecommendations === 'function'
        ? setAgentCategoryRecommendations
        : null;
      const translationCallRunner = this.runLlmRequest
        || (this.translationAgent && typeof this.translationAgent.runLlmRequest === 'function'
          ? this.translationAgent.runLlmRequest
          : null);
      this.translationCall = translationCall
        || (NT.TranslationCall && translationCallRunner
          ? new NT.TranslationCall({ runLlmRequest: translationCallRunner })
          : null);
    ...
    _toolGetNextUnits(args, job) {
      ...
      if (prefer !== 'range') {
        ...
        const resolveBatchSize = (categoryId) => {
          const key = this._normalizeCategory(categoryId || '');
          const cfg = key && batching[key] && typeof batching[key] === 'object'
            ? batching[key]
            : {};
          if (Number.isFinite(Number(cfg.batchSize))) {
            return Math.max(1, Math.min(15, Math.round(Number(cfg.batchSize))));
          }
          return 12;
        };
        const maxCharsPerUnit = 9000;
        for (let i = 0; i < blockRows.length && units.length < limit;) {
          ...
          const targetBatchSize = resolveBatchSize(rowCategoryId);
          const grouped = [];
          let groupedChars = 0;
          let j = i;
          while (j < blockRows.length && grouped.length < targetBatchSize) {
            const candidate = blockRows[j];
            if (!candidate || !candidate.blockId || consumedBlocks.has(candidate.blockId)) {
              break;
            }
            const candidateCategory = this._normalizeCategory(candidate.category || 'unknown') || 'unknown';
            if (candidateCategory !== rowCategoryId) {
              break;
            }
            if (prefer !== 'block' && resolveUnitKind(candidateCategory) === 'range' && rangeCoveredBlocks.has(candidate.blockId)) {
              break;
            }
            const candidateChars = Number.isFinite(Number(candidate.charCount)) ? Number(candidate.charCount) : String(candidate.originalText || '').length;
            if (grouped.length > 0 && (groupedChars + candidateChars) > maxCharsPerUnit) {
              break;
            }
            grouped.push(candidate);
            groupedChars += candidateChars;
            j += 1;
          }
          if (!grouped.length) {
            i += 1;
          ...
          units.push({
            unitType: 'block',
            id: first.blockId,
            blockIds: grouped.map((item) => item.blockId),
            originalText: first.originalText,
            categoryId: rowCategoryId,
            hasTranslation: grouped.every((item) => item.hasTranslation === true),
            batchSize: grouped.length
          });
          i = j;
        }
      }
    ...
    async _toolTranslateUnitStream(args, job, settings, { callId = null, source = 'model' } = {}) {
      ...
      if (!id && Array.isArray(input.blockIds) && input.blockIds.length) {
      ...
      let blockIds = [];
      if (unitType === 'block') {
        blockIds = Array.isArray(input.blockIds) && input.blockIds.length
          ? input.blockIds
      ...
      if (
        unitType === 'block'
        && blockIds.length > 1
        && this.translationCall
        && typeof this.translationCall.translateBatch === 'function'
      ) {
        return this._translateUnitStructuredBatch({
          id,
          blockIds,
          input,
          job,
    ...
    async _translateUnitStructuredBatch({ id, blockIds, input, job, settings, callId = null, source = 'model' } = {}) {
      const byId = job && job.blocksById && typeof job.blocksById === 'object' ? job.blocksById : {};
      const items = blockIds
        .map((blockId) => {
          const safeId = String(blockId || '').trim();
          if (!safeId || !Object.prototype.hasOwnProperty.call(byId, safeId)) {
            return null;
          }
          const block = byId[safeId];
          return {
            blockId: safeId,
            block,
            originalText: typeof block.originalText === 'string' ? block.originalText : '',
        ...
        .filter(Boolean);
      if (!items.length) {
        throw this._toolError('BAD_TOOL_ARGS', 'No resolvable blocks for structured batch');
      }
      ...
      const style = input.style === 'literal' || input.style === 'readable' || input.style === 'technical' || input.style === 'balanced'
        ? input.style
      ...
      const routeHint = runModels.preferredRoute === 'fast' || runModels.preferredRoute === 'strong'
        ...
        : 'auto';
      const routeUsed = input.route === 'fast' || input.route === 'strong'
        ? input.route
        : (routeHint === 'fast' || routeHint === 'strong' ? routeHint : 'fast');
      const glossary = Array.isArray(input.glossary) && input.glossary.length
        ? input.glossary
        : (job.agentState && Array.isArray(job.agentState.glossary) ? job.agentState.glossary : []);
      const contextSummary = typeof input.contextSummary === 'string' && input.contextSummary
        ? input.contextSummary
        : (job.agentState && typeof job.agentState.contextSummary === 'string' ? job.agentState.contextSummary : '');
      const batchGuidance = typeof input.batchGuidance === 'string' && input.batchGuidance
        ? input.batchGuidance
      ...
      const allowedModelSpecs = this._resolveAllowedModelSpecs({
        settings,
        runModels,
        requestedModel: typeof input.model === 'string' ? input.model : 'auto'
      });
      const batchCategoryCounts = {};
      items.forEach((item) => {
        batchCategoryCounts[item.category] = Number(batchCategoryCounts[item.category] || 0) + 1;
      });
      const signal = this.getJobSignal && job && job.id
        ? this.getJobSignal(job.id)
        : null;
      const attempt = items.reduce((max, item) => {
        const next = Number.isFinite(Number(item.block.translateAttempts))
          ? Number(item.block.translateAttempts) + 1
          : 1;
        item.block.translateAttempts = next;
        return Math.max(max, next);
      }, 1);
      const systemPrompt = [
        'Ты переводчик веб-страниц.',
        `Целевой язык: ${targetLang}.`,
        'Переводи только переданный текст, без ответов ассистента, приветствий и пояснений.',
        'Всегда возвращай строго валидный JSON формата {"items":[{"blockId":"...","text":"..."}],"report":{...}}.',
        `Стиль: ${style}.`,
      ...
      const batch = await this.translationCall.translateBatch(
        items.map((item) => ({
          blockId: item.blockId,
          originalText: item.originalText,
          category: item.category,
        ...
        })),
        {
          ...
          cacheEnabled: true,
          agentContext: {
            profile: runSettings && runSettings.profile ? runSettings.profile : (job.profile || 'auto'),
            style,
            routeHint: routeUsed,
      ...
      const resultMap = {};
      const translatedItems = Array.isArray(batch && batch.items) ? batch.items : [];
      translatedItems.forEach((row) => {
        if (!row || typeof row !== 'object') {
          return;
        }
        ...
        resultMap[blockId] = typeof row.text === 'string' ? row.text : '';
      });
      ...
      for (let i = 0; i < items.length; i += 1) {
        const item = items[i];
        const candidate = Object.prototype.hasOwnProperty.call(resultMap, item.blockId) ? resultMap[item.blockId] : '';
        if (!candidate || this._looksLikeAssistantFallback(item.originalText, candidate)) {
        ...
        await this.execute({
          ...
          blocks: null,
          settings,
          callId: `${callId || 'unit'}:${id}:${item.blockId}:final`,
          source: source === 'model' ? 'model' : 'system'
        });
        await this._markBlockDoneInternal({
          blockId: item.blockId,
          text: candidate,
          modelUsed: batch && batch.report && batch.report.meta && batch.report.meta.chosenModelSpec
            ? batch.report.meta.chosenModelSpec
      ...
      for (let i = 0; i < unresolved.length; i += 1) {
        ...
        try {
          const fallback = await this._toolTranslateBlockStream({
            blockId: item.blockId,
            targetLang,
            model: typeof input.model === 'string' ? input.model : undefined,
            style,
            ...
            callId: `${callId || 'unit'}:${id}:fallback:${item.blockId}`,
            source
          });
          const fallbackText = fallback && typeof fallback.text === 'string' ? fallback.text : '';
          if (!fallbackText || this._looksLikeAssistantFallback(item.originalText, fallbackText)) {
            errors.push({
              blockId: item.blockId,
              code: 'SUSPICIOUS_TRANSLATION',
              message: 'Fallback translation looked invalid'
            });
            continue;
          }
          await this._markBlockDoneInternal({
            blockId: item.blockId,
            text: fallbackText,
            modelUsed: fallback && fallback.modelUsed ? fallback.modelUsed : (allowedModelSpecs[0] || null),
            routeUsed: fallback && fallback.routeUsed ? fallback.routeUsed : routeUsed
          }, job, {
            qualityTag: 'raw',
            mode: 'execution'
          });
          results.push({
            ...
            modelUsed: fallback && fallback.modelUsed ? fallback.modelUsed : (allowedModelSpecs[0] || null),
            routeUsed: fallback && fallback.routeUsed ? fallback.routeUsed : routeUsed,
            skipped: Boolean(fallback && fallback.skipped === true)
          });
        } catch (error) {
          errors.push({
            blockId: item.blockId,
      ...
      return {
        ...
        id,
        categoryId: this._normalizeCategory(input.categoryId ? input.categoryId : '') || null,
        blockIds: items.map((item) => item.blockId),
        results,
    ...
    async _toolTranslateBlockStream(args, job, settings, { callId = null, source = 'model' } = {}) {
      ...
      if (this._isMemoryEnabled(settings) && this.translationMemoryStore) {
        ...
        if (
          cachedBlock
          && typeof cachedBlock.translatedText === 'string'
          && cachedBlock.translatedText
          && !this._looksLikeAssistantFallback(block.originalText, cachedBlock.translatedText)
        ) {
          await this.execute({
            name: 'page.apply_delta',
            arguments: {
      ...
      const signal = this.getJobSignal && job && job.id
        ...
        : null;
      const streamSupported = this._supportsStreamCapability();
      let rawJson = await this.runLlmRequest({
        tabId: Number.isFinite(Number(job.tabId)) ? Number(job.tabId) : null,
      ...
      let finalText = translatedBuffer
        || this._extractResponseText(rawJson)
        || (typeof block.originalText === 'string' ? block.originalText : '');
      if (this._looksLikeAssistantFallback(block.originalText, finalText)) {
        const strictInput = [
          {
            role: 'system',
            content: [{
              ...
              text: [
                'Ты переводчик.',
                `Переведи текст на язык ${targetLang}.`,
                'Верни только перевод исходной строки без комментариев, приветствий, JSON и markdown.',
                'Если строка является именем, заголовком или репликой, сохрани смысл и формат оригинала.'
              ].join(' ')
            }]
          },
          {
        ...
        try {
          const retryRoute = routeUsed === 'fast' ? 'strong' : routeUsed;
          const retryRaw = await this.runLlmRequest({
            tabId: Number.isFinite(Number(job.tabId)) ? Number(job.tabId) : null,
            taskType: 'translation_agent_execute_retry',
            request: {
              input: strictInput,
              maxOutputTokens: Math.max(180, Math.min(2200, Math.round(((block.originalText || '').length * 3.2) + 220))),
              temperature: 0,
              store: false,
              background: false,
              stream: false,
              signal,
              jobId: job.id || 'job',
              blockId: `exec_retry:${blockId}`,
              attempt: nextAttempt + 1,
              hintBatchSize: 1,
              agentRoute: retryRoute,
              allowedModelSpecs
            }
          });
          const retryText = this._extractResponseText(retryRaw);
          if (retryText && !this._looksLikeAssistantFallback(block.originalText, retryText)) {
            finalText = retryText;
            rawJson = retryRaw;
          }
        } catch (_) {
          // Keep initial candidate when retry fails.
        }
      }
      if (this._looksLikeAssistantFallback(block.originalText, finalText)) {
        finalText = typeof block.originalText === 'string' ? block.originalText : finalText;
      }
      translatedBuffer = finalText;
      await flushDelta(true);
    ...
    _looksLikeAssistantFallback(originalText, translatedText) {
      ...
      if (
        genericPhrases.some((phrase) => lower.includes(phrase))
        && !genericPhrases.some((phrase) => sourceLower.includes(phrase))
      ) {
        return true;
      }
      if (/^```/.test(translated)) {
      ...
      if (source.length > 0 && source.length <= 32 && translated.length > Math.max(120, source.length * 8)) {
        return true;
      }
      return false;
    }
    ...
    _extractResponseText(rawJson) {
      ...
      for (let i = 0; i < output.length; i += 1) {
        ...
        for (let j = 0; j < item.content.length; j += 1) {
          ...
          if (content && content.type === 'output_text' && typeof content.text === 'string' && content.text) {
            return content.text;
          }
        }
      }
