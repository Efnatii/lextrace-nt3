  ...
  class SettingsStore extends NT.ChromeLocalStoreBase {
    ...
    _legacyPatchToUserPatch(patch) {
      ...
      const mapProfile = (value) => {
        ...
        if (raw === 'minimal' || raw === 'medium' || raw === 'maximum' || raw === 'custom') {
          return raw;
        }
        if (raw === 'auto' || raw === 'balanced') {
          return 'medium';
        }
        if (raw === 'fast' || raw === 'bulk' || raw === 'readable') {
          return 'minimal';
        }
        if (raw === 'accurate' || raw === 'research' || raw === 'literal' || raw === 'technical') {
          return 'maximum';
        }
        return null;
      };
