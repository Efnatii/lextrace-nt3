...
(function initAgentSettingsPolicy(global) {
  ...
  const PROFILE_DEFAULTS = Object.freeze({
    minimal: {
      ...
      modelProfilePriority: ['gpt-4o-mini:standard', 'gpt-4.1-mini:standard', 'gpt-5-mini:standard'],
      memoryDefaults: {
        enabled: true,
        maxPages: 40,
        maxBlocks: 15000,
      ...
      toolConfigDefault: {
        ...
        'memory.build_glossary': 'on',
        'memory.update_context_summary': 'on'
      }
    },
    medium: {
      agentMode: 'agent',
    ...
    optimized: {
      ...
      compatCache: true,
      modelRoutingMode: 'profile_priority',
      modelProfilePriority: ['gpt-4.1-mini:standard', 'gpt-5-mini:standard', 'gpt-4o-mini:standard'],
      memoryDefaults: {
        enabled: true,
        maxPages: 80,
        maxBlocks: 30000,
  ...
  function normalizeProfileId(value, fallback = DEFAULT_USER_SETTINGS.profile) {
    const raw = typeof value === 'string' ? value.trim().toLowerCase() : '';
    if (!raw) {
      return fallback;
    }
  ...
  function resolveProfileMemoryDefaults(profileDefaults) {
    const src = profileDefaults && profileDefaults.memoryDefaults && typeof profileDefaults.memoryDefaults === 'object'
      ? profileDefaults.memoryDefaults
      : {};
    return {
      enabled: src.enabled !== false,
      maxPages: normalizeBoundedInteger(src.maxPages, DEFAULT_USER_SETTINGS.memory.maxPages, { min: 10, max: 5000 }),
      maxBlocks: normalizeBoundedInteger(src.maxBlocks, DEFAULT_USER_SETTINGS.memory.maxBlocks, { min: 50, max: 100000 }),
      maxAgeDays: normalizeBoundedInteger(src.maxAgeDays, DEFAULT_USER_SETTINGS.memory.maxAgeDays, { min: 1, max: 365 }),
      gcOnStartup: src.gcOnStartup !== false,
      ignoredQueryParams: normalizeIgnoredQueryParams(src.ignoredQueryParams)
    };
  }

  function normalizeUserSettings(input, { modelList = [] } = {}) {
    const src = input && typeof input === 'object' ? input : {};
    const normalizedProfile = normalizeProfileId(src.profile, DEFAULT_USER_SETTINGS.profile);
  ...
  function mergeToolDefaults(profileDefaults) {
    const base = cloneJson(PROFILE_DEFAULTS[DEFAULT_USER_SETTINGS.profile].toolConfigDefault, {});
  ...
  function getEffectiveSettings(userSettings, { modelList = [] } = {}) {
    ...
    const profileKey = PROFILE_IDS.includes(normalized.profile) ? normalized.profile : DEFAULT_USER_SETTINGS.profile;
    const profileDefaults = PROFILE_DEFAULTS[profileKey] || PROFILE_DEFAULTS[DEFAULT_USER_SETTINGS.profile];
    const mergedToolDefaults = mergeToolDefaults(profileDefaults);
    const profileMemoryDefaults = resolveProfileMemoryDefaults(profileDefaults);
    ...
    const defaultMemory = DEFAULT_USER_SETTINGS.memory;
    const normalizedMemoryIgnored = normalizeIgnoredQueryParams(normalized.memory.ignoredQueryParams);
    const defaultMemoryIgnored = normalizeIgnoredQueryParams(defaultMemory.ignoredQueryParams);
    const memoryEnabledUsesProfileDefault = normalized.memory.enabled === defaultMemory.enabled;
    const memoryMaxPagesUsesProfileDefault = normalized.memory.maxPages === defaultMemory.maxPages;
    const memoryMaxBlocksUsesProfileDefault = normalized.memory.maxBlocks === defaultMemory.maxBlocks;
    const memoryMaxAgeUsesProfileDefault = normalized.memory.maxAgeDays === defaultMemory.maxAgeDays;
    const memoryGcUsesProfileDefault = normalized.memory.gcOnStartup === defaultMemory.gcOnStartup;
    const memoryIgnoredUsesProfileDefault = areStringArraysEqual(normalizedMemoryIgnored, defaultMemoryIgnored);

    const effectiveMemoryEnabled = memoryEnabledUsesProfileDefault
      ? profileMemoryDefaults.enabled !== false
      : normalized.memory.enabled !== false;
    const effectiveMemoryMaxPages = memoryMaxPagesUsesProfileDefault
      ? profileMemoryDefaults.maxPages
      : normalized.memory.maxPages;
    const effectiveMemoryMaxBlocks = memoryMaxBlocksUsesProfileDefault
      ? profileMemoryDefaults.maxBlocks
      : normalized.memory.maxBlocks;
    const effectiveMemoryMaxAgeDays = memoryMaxAgeUsesProfileDefault
      ? profileMemoryDefaults.maxAgeDays
      : normalized.memory.maxAgeDays;
    const effectiveMemoryGcOnStartup = memoryGcUsesProfileDefault
      ? profileMemoryDefaults.gcOnStartup !== false
      : normalized.memory.gcOnStartup !== false;
    const effectiveMemoryIgnored = memoryIgnoredUsesProfileDefault
      ? profileMemoryDefaults.ignoredQueryParams.slice()
      : normalizedMemoryIgnored.slice();

    if (!memoryEnabledUsesProfileDefault && effectiveMemoryEnabled !== (profileMemoryDefaults.enabled !== false)) {
      addOverride(overrides, 'memory.enabled', effectiveMemoryEnabled);
    }
    if (!memoryMaxPagesUsesProfileDefault && effectiveMemoryMaxPages !== profileMemoryDefaults.maxPages) {
      addOverride(overrides, 'memory.maxPages', effectiveMemoryMaxPages);
    }
    if (!memoryMaxBlocksUsesProfileDefault && effectiveMemoryMaxBlocks !== profileMemoryDefaults.maxBlocks) {
      addOverride(overrides, 'memory.maxBlocks', effectiveMemoryMaxBlocks);
    }
    if (!memoryMaxAgeUsesProfileDefault && effectiveMemoryMaxAgeDays !== profileMemoryDefaults.maxAgeDays) {
      addOverride(overrides, 'memory.maxAgeDays', effectiveMemoryMaxAgeDays);
    }
    if (!memoryGcUsesProfileDefault && effectiveMemoryGcOnStartup !== (profileMemoryDefaults.gcOnStartup !== false)) {
      addOverride(overrides, 'memory.gcOnStartup', effectiveMemoryGcOnStartup);
    }
    if (!memoryIgnoredUsesProfileDefault && !areStringArraysEqual(effectiveMemoryIgnored, profileMemoryDefaults.ignoredQueryParams)) {
      addOverride(overrides, 'memory.ignoredQueryParams', effectiveMemoryIgnored.slice());
    }
    ...
    const effective = {
      ...
      memory: {
        ...
        maxPages: effectiveMemoryMaxPages,
        maxBlocks: effectiveMemoryMaxBlocks,
        maxAgeDays: effectiveMemoryMaxAgeDays,
        gcOnStartup: effectiveMemoryGcOnStartup,
        ignoredQueryParams: effectiveMemoryIgnored.slice()
      },
      ui: {
      ...
      legacyProjection: {
        ...
        translationMemoryEnabled: effectiveMemoryEnabled,
        translationMemoryMaxPages: effectiveMemoryMaxPages,
        translationMemoryMaxBlocks: effectiveMemoryMaxBlocks,
        translationMemoryMaxAgeDays: effectiveMemoryMaxAgeDays,
        translationMemoryGcOnStartup: effectiveMemoryGcOnStartup,
        translationMemoryIgnoredQueryParams: effectiveMemoryIgnored.slice(),
