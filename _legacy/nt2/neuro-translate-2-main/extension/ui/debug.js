(function initDebug(global) {
  ...
  function normalizeRoute(route) {
    const keyRaw = safeString(route || '', '').replace(/^#/, '').trim().toLowerCase();
    const key = keyRaw === 'diff-patches' ? 'diff' : keyRaw;
    const allowed = ['overview', 'plan', 'tools', 'diff', 'categories', 'memory', 'ratelimits', 'perf', 'security', 'export'];
  ...
  class DebugApp {
    ...
    async init(initialTabId) {
      this.initialTabId = normalizeTabId(initialTabId);
      this._cacheElements();
      ...
      this.client.setHelloContext({ tabId: this.initialTabId });
      this.client.connect();

      this.route = await this._resolveInitialRoute();
      this._initTabs();
      this._scheduleRender();
    }
    ...
    _cacheElements() {
      ...
      this.fields.headerActions = this.doc.querySelector('[data-field="header-actions"]');
    ...
    _buildHeaderActions() {
      if (!this.fields.headerActions) {
        return;
      }
      Ui.clearNode(this.fields.headerActions);
      const items = [
        {
          action: 'export-json',
          icon: 'export',
          label: I18n.t('debug.btnExportJson', 'Экспорт отчета JSON'),
          tooltip: I18n.t('tooltips.exportJson', 'Скачать redacted JSON-отчет.')
        },
        {
          action: 'export-html',
          icon: 'export',
          label: I18n.t('debug.btnExportHtml', 'Экспорт HTML'),
          tooltip: I18n.t('tooltips.exportHtml', 'Скачать HTML-отчет для передачи коллегам.')
        },
        {
          action: 'copy-diagnostics',
          icon: 'copy',
          label: I18n.t('debug.btnCopyDiagnostics', 'Копировать диагностику'),
          tooltip: I18n.t('tooltips.diagnostics', 'Создает и копирует redacted-диагностику.')
        },
        {
          action: 'kick-scheduler',
          icon: 'refresh',
          label: I18n.t('debug.btnKickScheduler', 'Пнуть планировщик'),
          tooltip: I18n.t('tooltips.debugKick', 'Запускает scheduler tick принудительно.')
        }
      ];
      items.forEach((item) => {
        this.fields.headerActions.appendChild(Ui.createIconButton({
          icon: item.icon,
          label: item.label,
    ...
    async _resolveInitialRoute() {
      const fromLocation = resolveInitialRoute(global.location);
      if (fromLocation !== 'overview') {
        return fromLocation;
      }
      if (!this.uiStateStore || typeof this.uiStateStore.getDebugState !== 'function') {
        return fromLocation;
      }
      try {
        const state = await this.uiStateStore.getDebugState();
        if (state && state.activeTab) {
          return normalizeRoute(state.activeTab);
        }
      } catch (_) {
        // ignore state-store errors
      }
      return fromLocation;
    }

    _saveUiState(patch) {
      if (!this.uiStateStore || typeof this.uiStateStore.setDebugState !== 'function') {
        return;
      }
      this.uiStateStore.setDebugState(patch).catch(() => {});
    }

    _initTabs() {
      this.tabs = new Ui.TabsController(this.root, {
        defaultTab: this.route,
        hashMode: true,
        normalizeHashTab: normalizeRoute,
        serializeHashTab: (tabId) => (tabId === 'diff' ? 'diff-patches' : tabId),
        onChange: (tabId) => {
          this.route = normalizeRoute(tabId);
          this._saveUiState({ activeTab: this.route });
          this._scheduleRender();
        }
      });
      this.tabs.setActive(this.route, { emit: false, fromHash: false });
    }
    ...
    async _handleAction(action) {
      ...
      if (action === 'cancel-job') {
        const tabId = this._requireTabId();
        if (!tabId) {
          return;
        }
    ...
    _renderHeader() {
      ...
      Ui.setText(this.fields.headerJob, `Вкладка: ${tabId === null ? '-' : tabId} | Задача: ${jobId}`);
      Ui.setText(this.fields.headerState, `Статус: ${safeString(job && job.status, '-')} | Этап: ${safeString(job && job.runtime && job.runtime.stage, '-')}`);
      Ui.setText(
        this.fields.headerRuntime,
        `Аренда: ${formatTs(job && job.runtime && job.runtime.leaseUntilTs)} | Повтор: ${formatTs(job && job.runtime && job.runtime.nextRetryAtTs)} | Offscreen: ${safeString(offscreen.connected, '-')} | Активные запросы: ${Array.isArray(scheduler.activeJobs) ? scheduler.activeJobs.length : 0}`
      );
    }
    ...
    _renderOverview() {
      ...
      const rows = [
        ...
        ['DOM хэш', safeString(this.snapshot.domHash || job.domHash, '-')],
        ['Этап', safeString(runtime.stage || job.status, '-')],
        ['Статус', safeString(job.status, '-')],
        ['Аренда до', formatTs(runtime.leaseUntilTs)],
        ['Повтор в', formatTs(runtime.nextRetryAtTs)],
        ['Попытка', Number.isFinite(Number(runtime.attempt)) ? Number(runtime.attempt) : 0],
        ['Прогресс', `${progress}%`],
        ['Ошибок', failed],
        ['Активных запросов', this._activeRequestsCount()]
      ];
    ...
    _renderPlan() {
      ...
      Ui.setText(this.fields.planHint, `Таксономия: ${categories.length} категорий`);
      Ui.clearNode(this.fields.planCategories);
      ...
      if (mapping && Object.keys(mapping).length) {
        ...
        Ui.setText(this.fields.planMapping, 'Сводка mapping: нет данных');
      }
    }
      ...
      if (this.fields.toolsFilterName) {
        ...
        names.forEach((name) => {
          const option = Ui.createElement('option', {
            text: name === 'all' ? 'инструмент: все' : name,
            attrs: { value: name }
          });
          if (name === currentName) {
    ...
    _renderDiffPatches() {
      ...
      filteredDiff.slice(-800).forEach((item) => {
        ...
        const row = Ui.createElement('div', {
          ...
          text: `${key || '-'} | ${safeString(item.category || item.categoryId, '-')} | ${safeString(item.qualityTag || item.status, '-')} | длина ${safeString((item.originalText || '').length, '0')}/${safeString((item.translatedText || '').length, '0')}`
        });
      ...
      Ui.setText(this.fields.diffMeta, selected
        ? `Модель: ${safeString(selected.modelUsed, '-')} | Маршрут: ${safeString(selected.routeUsed, '-')} | Обновлено: ${formatTs(selected.updatedAt || selected.ts)}`
        : 'Модель: - | Маршрут: - | Обновлено: -');
    ...
    _renderMemory() {
      ...
      this._renderKv(this.fields.memoryKv, [
        ['Включена', settings.translationMemoryEnabled === false ? 'false' : 'true'],
        ['Ключ страницы', safeString(memoryRestore.pageKey, '-')],
        ['Восстановлено в', formatTs(memoryRestore.ts)],
        ['Попаданий', Number(memoryRestore.hits || 0)]
      ]);
    }
    ...
    _renderRateLimits() {
      ...
      this._renderKv(this.fields.ratelimitsKv, [
        ['Моделей', keys.length],
        ['Соединение', safeString(this.uiStatus.state, '-')]
      ]);
    ...
    _renderPerf() {
      ...
      this._renderKv(this.fields.perfKv, [
        ['Сканирование (мс)', Number(perf.scanMs || 0)],
        ['Классификация (мс)', Number(perf.classifyMs || 0)],
        ['Средняя delta latency', Number(perf.avgDeltaLatency || 0)],
        ['Попытки rebind', Number(perf.rebindAttempts || 0)],
        ['Оценка storage', Number(perf.storageEstimate || 0)]
      ]);
    ...
    _renderSecurity() {
      ...
      Ui.setText(this.fields.securitySummary, `Учетные данные: ${shortText(JSON.stringify(security.credentials || {}), 220)}`);
    ...
    _tabId() {
      const direct = normalizeTabId(this.snapshot && this.snapshot.tabId);
      if (direct) {
        return direct;
      }
      return normalizeTabId(this.initialTabId);
    }
    ...
    _requireTabId() {
      ...
      this.toasts.show('Не удалось определить tabId текущей страницы.', { tone: 'warn' });
      return null;
    }
  ...
  (async () => {
    ...
    await app.init(resolveInitialTabId());
  })();
})(globalThis);
