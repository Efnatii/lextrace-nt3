(function initPopupViewModel(global) {
  ...
  function resolveStage(job) {
    ...
    const runtimeStage = job && job.runtime && typeof job.runtime.stage === 'string'
      ...
      : '';
    if (runtimeStage) {
      if (runtimeStage === 'translating' || runtimeStage === 'execution_in_progress' || runtimeStage === 'execution') {
        return 'running';
      }
      if (runtimeStage === 'scanning') {
        return 'preparing';
      }
      if (runtimeStage === 'analysis_in_progress' || runtimeStage === 'analysis' || runtimeStage === 'planning_in_progress') {
        return 'planning';
      }
      if (runtimeStage === 'proofreading_in_progress' || runtimeStage === 'proofreading') {
        return 'proofreading';
      }
      return runtimeStage;
    }
    if (status === 'running' || status === 'preparing' || status === 'planning' || status === 'completing' || status === 'proofreading') {
      return status;
    }
    return status || 'idle';
  }
  ...
  function inferCategoryFlow(categoryId, { plan, runtimeTuning } = {}) {
    const id = normalizeCategoryId(categoryId);
    const safePlan = plan && typeof plan === 'object' ? plan : {};
    const parallelCategories = Array.isArray(safePlan.parallelCategories)
      ? safePlan.parallelCategories.map(normalizeCategoryId).filter(Boolean)
      : [];
    const sequentialCategories = Array.isArray(safePlan.sequentialCategories)
      ? safePlan.sequentialCategories.map(normalizeCategoryId).filter(Boolean)
      : [];
    const parallelSet = new Set(parallelCategories);
    const sequentialSet = new Set(sequentialCategories);

    if (id && sequentialSet.has(id) && !parallelSet.has(id)) {
      return 'sequential';
    }
    if (id && parallelSet.has(id) && !sequentialSet.has(id)) {
      return 'parallel';
    }

    const token = String(id || '');
    if (/head|heading|title|table|code|quote|footnote/.test(token)) {
      return 'sequential';
    }
    if (/paragraph|content|list|meta|label|button|link|main/.test(token)) {
      return 'parallel';
    }

    const tuning = runtimeTuning && typeof runtimeTuning === 'object' ? runtimeTuning : {};
    const parallelism = safeString(
      tuning.parallelismOverride !== undefined ? tuning.parallelismOverride : tuning.parallelism,
      'auto'
    ).trim().toLowerCase();
    if (parallelism === 'low') {
      return 'sequential';
    }
    if (parallelism === 'high' || parallelism === 'mixed') {
      return 'parallel';
    }
    return 'parallel';
  }
  ...
  function buildCategoryList(job, agentState) {
    ...
    const agentPlan = safeAgent.plan && typeof safeAgent.plan === 'object' ? safeAgent.plan : null;
    const runtimeTuning = safeAgent.runtimeTuning && typeof safeAgent.runtimeTuning === 'object'
      ? safeAgent.runtimeTuning
      : null;
    ...
    const optionsFromQuestion = question && Array.isArray(question.options)
      ? question.options
      : [];
    const jobStatus = safeString(safeJob.status, '').trim().toLowerCase();
    const runtimeStage = safeString(safeJob.runtime && safeJob.runtime.stage, '').trim().toLowerCase();
    const awaitingCategories = jobStatus === 'awaiting_categories' || runtimeStage === 'awaiting_categories';
    const restrictToQuestion = awaitingCategories && optionsFromQuestion.length > 0;
    ...
    if (restrictToQuestion) {
      selected.forEach((id) => {
        ...
        put(id, {
          titleRu: id,
          countUnits: Number.isFinite(Number(countsByCategory[id])) ? Number(countsByCategory[id]) : 0
        });
      });
    } else {
    ...
    const items = ids.map((id) => {
      ...
      return {
        ...
        mode,
        translationFlow: inferCategoryFlow(id, { plan: agentPlan, runtimeTuning }),
  ...
  function computeViewModel(snapshot, uiStatus = {}) {
    ...
    return {
      tabId: Number.isFinite(Number(src.tabId)) && Number(src.tabId) > 0 ? Number(src.tabId) : null,
      stage,
